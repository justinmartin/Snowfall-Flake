...
in {
  ...
  config = mkIf cfg.enable {
    ...
    # frgd.home.configFile."waybar/config".source = ./config;
    frgd.home.configFile."waybar/style.css".source = ./style.css;

    home-manager.users.justin = { # Home-manager waybar config
      programs.waybar = {
        enable = true;
        systemd = {
          enable = true;
