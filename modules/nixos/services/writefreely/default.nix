...
let
  ...
  cfg = config.frgd.services.writefreely;
...
in
{
  options.frgd.services.writefreely = {
    enable =
    ...
    database = {
      ...
      createLocally = mkOption {
        ...
        default = false;
        description = lib.mdDoc ''
          When `frgd.services.writefreely.database.type` is set to
  ...
  config = mkIf cfg.enable {
    assertions = [
      {
        assertion = cfg.host != "";
        message = "frgd.services.writefreely.host must be set";
      }
      {
        assertion = isMysqlLocal -> cfg.database.passwordFile != null;
        message =
          "frgd.services.writefreely.database.passwordFile must be set if frgd.services.writefreely.database.createLocally is set to true";
      }
      {
        assertion = isSqlite -> !cfg.database.createLocally;
        message =
          "frgd.services.writefreely.database.createLocally has no use when frgd.services.writefreely.database.type is set to sqlite3";
      }
    ];
