...
let
  ...
  inherit (lib.frgd) mkBoolOpt mkOpt;
  inherit (pkgs) fortune toot;
  inherit (pkgs.snowfallorg) cow2img;

  cfg = config.frgd.services.cowsay-mastodon-poster;
...
in
{
  options.frgd.services.cowsay-mastodon-poster = with types; {
    enable = mkBoolOpt false "Whether or not to enable cowsay posts.";
    short = mkBoolOpt false "Use short fortunes only.";
    user = mkOpt str config.frgd.user.name "The user to run as.";
